...
async def lifespan(app: FastAPI):
    ...
    """
    # Startup: Pre-load the agent and reload dashboard data
    print("Starting up... Pre-loading agent and dashboard data...")
    
    # Reload dashboard data from Excel to ensure fresh data
    try:
        reload_data()
    except Exception as e:
        print(f"⚠ Warning: Could not reload dashboard data: {e}")
...
async def read_root():
    ...
    # Reload data to ensure we have the latest from Excel
    try:
        reload_data()
    except Exception as e:
        print(f"Warning: Could not reload dashboard data: {e}")
...
<body><h1>Template file not found</h1></body></html>"""
    ...
    # Generate Google Analytics script if GA4_MEASUREMENT_ID is set
    ga4_measurement_id = os.getenv('GA4_MEASUREMENT_ID', '').strip()
    if ga4_measurement_id:
        ga4_script = f"""<!-- Google Analytics (GA4) -->
...
<script>
  ...
  gtag('config', '{ga4_measurement_id}');
        ...
        ga4_script = "<!-- Google Analytics not configured. Set GA4_MEASUREMENT_ID environment variable to enable. -->"
